...
import {
  cambiarEstadoMulta,
  getMultasPorCedulaUsuario,
} from "../../../services/multaServices.js";
...
const initialOptions = {
  "client-id":
    "Adoww9KeExfepE0evtSMzMHBZqgFYTOitCqUjfQaLt1Np1V7gY9P-v-kPO-FzrBeMt4IsCDw0qC9tzqQ",
  currency: "USD",
  intent: "capture",
};
...
const MisMultas = () => {
  ...
  function openPopup(type) {
    setPopupContent({
      type,
      title:
        type === "dispute" ? "Presentar Disputa" : "Realizar Pago de Multa",
    });
  ...
  async function handleSubmit(event) {
    ...
    try {
      const disputaCreada = await createDisputa({
        ...disputeDataRef.current,
        numeroPlaca: multa.numeroPlaca,
      });
      ...
      const emailMessage = `
      Se ha presentado una disputa para la multa con ID ${formatId(
        multa.idMulta
      )}.
  ...
  `;
      ...
      await sendEmail(disputaCreada.juezAsignadoCorreo, emailMessage);

      closePopup();
      //   window.location.reload();
      await fetchData();

      console.log("disputa creada", disputaCreada);
      console.log("ofcicial id", oficialData.email);
    } catch (error) {
  ...
  const DisputePopup = () => (
    <div className="misMultas-popup">
      <div className="misMultas-popup-content">
        <span className="misMultas-close" onClick={closePopup}>
          &times;
        </span>
          ...
          <table className="misMultas-payment-summary">
            <tbody>
              ...
              <tr>
                <td>Total</td>
                <td>
                  {formatCurrency(multa.montoTotal + (multa?.montoMora || 0))}
                </td>
              </tr>
            </tbody>
          </table>
  ...
  const showAbrirDisputa = (multa) => {
    return disputas.find((disputa) => disputa.multaId === multa.idMulta)
      ? false
      : true;
  };
      ...
      <div className="misMultas-main-container">
        <div className="misMultas-grid-container">
          {filteredMultas.length === 0 ? (
            <p className="misMultas-no-multas-message">
            ...
            filteredMultas.map((multa) => (
              <div key={multa.id} className="misMultas-card">
                <div className="misMultas-infoMulta">
                  <p>
                    <strong>ID Multa:</strong> {formatId(multa.idMulta)}
                  </p>
                  <p>
                    <strong>Vehículo:</strong> {multa.numeroPlaca}
                  </p>
                  <p>
                    <strong>Fecha:</strong>{" "}
                  ...
                  <p>
                    <strong>Monto:</strong> {formatCurrency(multa.montoTotal)}
                  </p>
                </div>
                <div className="misMultas-button-container">
                  {showAbrirDisputa(multa) && (
                    <Button
                      onClick={() => {
                        openPopup("dispute");
                        setDataMulta(multa);
